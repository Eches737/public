...
async function run() {
  ...
  const html = `<!doctype html><html><head></head><body>
    ...
    <script>window.location.search='?q=cnt'</script>
    <script id="app">${code}</script>
  </body></html>`;

  const dom = new JSDOM(html, { runScripts: 'dangerously', resources: 'usable', url: 'http://localhost:3000/search-results.html' });
  ...
  await new Promise((resolve, reject) => {
    ...
    dom.window.addEventListener('paperscout:ready', () => {});
    // Poll for results
    (function waitForResults(attempts){
      ...
      if (attempts++ > 40) return reject(new Error('No rendered items'));
